...
// Fetch all chats (for navigation)
$stmt = $dbConnection->prepare("SELECT c.id AS chat_id, u.name AS chat_partner FROM chats c
...
// If chat_id is passed, fetch messages for that chat
$messages = [];
...
<body>
  <div class="chat-container">
    ...
    <div class="chat-list">
      ...
      <?php else: ?>
        <div id="firstChat" class="chat-item" data-chat-id="<?php echo $chats[0]['chat_id']; ?>">
          <h3><?php echo htmlspecialchars($chats[0]['chat_partner']); ?></h3>
        </div>
        <?php for ($i = 1; $i < count($chats); $i++): ?>
          <div class="chat-item" data-chat-id="<?php echo $chats[$i]['chat_id']; ?>">
      ...
      <div id="chatWindow" class="chat-window">
        <?php if (!empty($messages)): ?>
          <?php foreach ($messages as $message): ?>
            <div
              class="message <?php echo ($message['sender_name'] == $_SESSION['user_name']) ? 'message-sender' : 'message-receiver'; ?>">
